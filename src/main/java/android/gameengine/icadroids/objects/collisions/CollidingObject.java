...
public class CollidingObject {
     ...
     *
     * @param startX          The current x position of the object
     * @param startY          The current y position of the object
     * @param endX            The next x position of the object
     * @param endY            The next y position of the object
     * @param sprite          The sprite of the object
     * @param gameTiles       The gameTiles object to check the collision with
     * @param collisionObject An ICollision object that needs to be called when a collision
     *                        has occurred
    ...
    public void calculateCollision(double endX, double endY,
                                   ...
                                   ICollision collisionObject) {    // no tiles, no sprite: no tile collisions
        ...
        {    // find index of gridline just left of start-left side of item
            ...
            xCollisionSide = 1;
        } else {    // find index of gridline just right of start-right side of item
        ...
        while (xFactor < 1 || yFactor < 1) {    // handle collision that comes first, that is the lower factor (<1 guaranteed by loop criterion)
            if (xFactor <= yFactor) {    // horizontal collision occurrs earlier than vertical
                if ((xTileIndex >= 0) && (xTileIndex < gameTiles.getMapWidth())) {    // find matching ypos, then tiles in range [ypos, ypos+frameheight]
                    ...
                    for (int yindex = firsttile; yindex <= lasttile; yindex++) {    // see if there is a tile at the current position
                ...
                if (moveleft) {    // move collision gridline to the left for next check
                    xTileIndex--;
                    collisionX = (xTileIndex + 1) * gridsize /*+ mapStartX*/;
                    itemXatCollision = collisionX;
                } else {    // move collision gridline to the right for next check
                    xTileIndex++;
                    collisionX = xTileIndex * gridsize /*+ mapStartX*/;
                    itemXatCollision = collisionX - objwidth;
                }
                ...
                xFactor = calcMoveFactor(startX, endX, itemXatCollision);
            } else {    // vertical collision occurs earlier (comments omitted, are like horizontal collision)
     ...
     *
     * @param pos   a number, usually an X- or Y-position
     * @param deler the divisor, usually tilesize
    ...
    private double calcMoveFactor(double start, double end, int pos) {
        if (end == start) {    // the calculation is not quite right (since (colx-prevx)/0 is not equal to 1....
