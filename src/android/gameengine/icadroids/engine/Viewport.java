...
public class Viewport {
     ...
     * At the moment this is always 0, but the variable is kept in (possible change?)
     */
    private int minX;
     ...
     * Since minX is zero at the moment, this equals the width of the game world.
     */
    private int maxX;

    /**
     * Bottom edge of the game world.<br />
     * Since minX is zero at the moment, this equals the height of the game world.
     */
    private int maxY;
     ...
     * This is needed to give the viewportX and -Y a starting value
     */
	...
	if (vpositioning == PLAYER_TOP) {
	    ...
	    viewportY = player.getY()
		    - (viewportHeight - (int)(player.getFrameHeight()*zoomFactor)) / 2;
	} else if (vpositioning == PLAYER_BOTTOM) {
	    viewportY = player.getY()
	...
	if (hpositioning == PLAYER_LEFT) {
	    ...
	    viewportX = player.getX()
		    - (viewportWidth - (int)(player.getFrameWidth()*zoomFactor)) / 2;
	} else if (hpositioning == PLAYER_RIGHT) {
	    viewportX = player.getX()
    ...
    /**
     * Return rectangle for the (background)image. The Image is scaled to
     * this rec, visible part is cut out 
     * 
     * @return target rectangle to scale the image, 
     * 		from which the visible part is cut
     */
    final Rect getDrawMask() {
	return new Rect(
		minX - Math.round(viewportX * zoomFactor),
		minY - Math.round(viewportY * zoomFactor),
    ...
    /**
     * Get translation for the x-coordinates (shift in drawing: Canvas always draws
     * from coordinate 0, so you shift position the other way)
     * 
    ...
    */
    ...
    /**
     * Get translation for the y-coordinates (shift in drawing: Canvas always draws
     * from coordinate 0, so you shift position the other way)
     * 
